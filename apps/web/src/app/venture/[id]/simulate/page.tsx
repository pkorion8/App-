      ...
      {!run ? (
        ...
        <div className="mt-6 space-y-4">
          ...
          <VentureModeSection mode="pro" className="space-y-4">
            ...
            <section className="grid gap-4 lg:grid-cols-2"><Card><p className="text-xs font-semibold uppercase tracking-[.18em] text-vs-fg-muted">Checkpoints & alternate timelines</p><p className="mt-2 text-sm text-vs-fg-muted">Use a saved checkpoint to branch from an earlier decision. Standard mode is limited to three rewinds; Reality Mode disables rewinds.</p><div className="mt-4"><CheckpointPanel ventureId={venture.id} runId={run.id} checkpoints={checkpoints} currentDay={run.virtual_day} realityMode={Boolean(branchRun?.reality_mode)} rewindCount={branchRun?.rewind_count ?? 0} /></div></Card><Card><p className="text-xs font-semibold uppercase tracking-[.18em] text-vs-fg-muted">Recent events</p><div className="mt-3 space-y-3">{events.length > 0 ? events.map((event) => <div key={event.id} className="border-l-2 border-vs-border pl-3"><p className="text-xs text-vs-fg-muted">Day {event.virtual_day} · {event.event_type.replaceAll("_", " ")}</p><p className="mt-1 text-sm leading-6 text-vs-fg">{event.description}</p></div>) : <p className="text-sm text-vs-fg-muted">No simulation events recorded yet.</p>}</div></Card></section>
            {run.stage === "complete" && <Card><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold text-vs-fg">This timeline is complete.</p><p className="mt-1 text-sm text-vs-fg-muted">Use the resulting state and decisions to scope the first build package.</p></div><Link href={`/venture/${venture.id}/build`} className="rounded-vs-sm bg-vs-primary px-4 py-2 text-sm font-semibold text-vs-primary-fg">Open Build Studio →</Link></div></Card>}
          </VentureModeSection>
        </div>
      )}
    </main>
...
function SimpleStat({ label, value, help }: { label: string; value: string; help: string }) { return <div className="rounded-2xl bg-vs-bg-subtle p-3"><p className="text-[11px] text-vs-fg-muted">{label}</p><p className="mt-1 text-lg font-semibold text-vs-fg">{value}</p><p className="mt-1 text-[10px] leading-4 text-vs-fg-muted">{help}</p></div>; }
function Stat({ label, value }: { label: string; value: string }) { return <div className="rounded-vs-md bg-vs-bg-subtle p-3"><p className="text-[11px] text-vs-fg-muted">{label}</p><p className="mt-1 text-base font-semibold text-vs-fg">{value}</p></div>; }
