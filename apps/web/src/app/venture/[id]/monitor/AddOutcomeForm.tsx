...
const METRIC_LABEL: Record<(typeof METRIC_TYPES)[number], string> = {
  ...
  milestone: "Milestone completion (0/1 or %)",
  other: "Other",
};

function SubmitButton() {
  const { pending } = useFormStatus();
  return <Button type="submit" disabled={pending}>{pending ? "Saving..." : "Add real observation"}</Button>;
}
  ...
  return (
    <form action={formAction} className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div><Label htmlFor="metricValue">Value</Label><Input id="metricValue" name="metricValue" type="number" step="any" min="0" required /><FieldError>{state.fieldErrors?.metricValue}</FieldError></div>
      </div>
