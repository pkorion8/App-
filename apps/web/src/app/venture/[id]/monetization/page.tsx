...
export default async function Monetization({ params }: { params: Promise<{ id: string }> }) {
  ...
  const cost = (build?.cost_estimate || {}) as { totalMonthly?: number | null };
  const hasLegacyNumericPricing = typeof cost.totalMonthly === "number";
  const cards = createMonetizationExperiments({ geography: v.geography, audience: v.target_user, product: shape?.problem_statement || v.raw_idea_text, pricingModel: shape?.pricing_model, hasCompetitorPricing: false });
  ...
  return <main className="mx-auto max-w-6xl p-6">
    <VentureModeSection mode="simple">
      <p className="text-xs font-semibold uppercase tracking-[.2em] text-vs-primary">Step 3 · Money</p>
      ...
      <Card className="mt-6 border-vs-primary/20 bg-vs-primary/5"><div className="grid gap-3 sm:grid-cols-2"><PlainInput title="Current idea" value={shape?.pricing_model ? simpleModelName(shape.pricing_model) : "No money model chosen yet"}/><PlainInput title="Vendor pricing status" value={hasLegacyNumericPricing ? "Legacy estimate only — verify current pricing" : "Not connected"}/></div><p className="mt-3 text-xs text-vs-fg-muted">We do not have live vendor pricing or evidence showing what customers are actually willing to pay. Historical Build Studio cost estimates are not used as current pricing evidence here.</p></Card>
      ...
      <Card className="mt-5"><p className="text-lg font-semibold text-vs-fg">The goal is not to guess the perfect price.</p><p className="mt-2 text-sm leading-6 text-vs-fg-muted">Choose one reasonable way to make money, simulate it, then replace assumptions with real customer behavior later.</p>{keys.size > 0 ? <Link href={`/venture/${id}/simulate`} className="mt-4 inline-flex rounded-vs-sm bg-vs-primary px-4 py-2 text-sm font-semibold text-vs-primary-fg">Continue to simulation →</Link> : <p className="mt-4 text-xs font-semibold text-vs-fg-muted">Choose at least one option above before moving to the simulation.</p>}</Card>
    </VentureModeSection>
    ...
    <VentureModeSection mode="pro">
      ...
      <Card className="mt-6"><div className="grid gap-3 sm:grid-cols-3"><Mini label="Current model" value={shape?.pricing_model?.replaceAll("_", " ") || "Not decided"}/><Mini label="Vendor cost evidence" value={hasLegacyNumericPricing ? "Legacy estimate only" : "Not connected"}/><Mini label="Competitor pricing" value="UNAVAILABLE in connected sources"/></div><p className="mt-3 text-xs text-vs-fg-muted">These inputs constrain the hypotheses below. They do not establish willingness to pay. Stored historical cost numbers are excluded from the evidence because Build Studio has no live pricing source.</p></Card>
      ...
      {keys.size > 0 && <div className="mt-6 flex justify-end"><Link href={`/venture/${id}/simulate`} className="rounded-vs-sm bg-vs-primary px-4 py-2 text-sm font-semibold text-vs-primary-fg">Run the venture simulation →</Link></div>}
    </VentureModeSection>
  </main>;
}
