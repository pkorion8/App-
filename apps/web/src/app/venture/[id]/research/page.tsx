...
export default async function ResearchPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ again?: string }> }) {
  ...
  const missionTargetUser = mission?.target_user || venture.target_user || "the selected audience";
  const missionGeography = mission?.geography || venture.geography || "the selected geography";
    ...
    <main className="mx-auto max-w-6xl p-4 sm:p-6">
      ...
      {showForm ? (
        ...
        <div className="mt-6 space-y-5">
          {!hasFindings ? <NoFindings ventureId={venture.id} targetUser={missionTargetUser} geography={missionGeography} /> : null}
          <VentureModeSection mode="simple" className="space-y-4">
            <Card className="border-vs-primary/20 bg-vs-primary/5"><div className="flex flex-wrap items-center justify-between gap-3"><div><Badge status={hasFindings ? "primary" : "neutral"}>{hasFindings ? "WHAT WE FOUND" : "NO LIVE FINDINGS YET"}</Badge><p className="mt-2 text-sm text-vs-fg">{hasFindings ? <>We checked this idea for <strong>{missionTargetUser}</strong> in <strong>{missionGeography}</strong>.</> : <>The connected sources returned no usable evidence for <strong>{missionTargetUser}</strong> in <strong>{missionGeography}</strong>. We did not replace the missing evidence with generated estimates.</>}</p></div><Link href={`/venture/${venture.id}/research?again=1`} className="text-sm font-medium text-vs-primary">Check again →</Link></div></Card>
          ...
          <VentureModeSection mode="pro" className="space-y-5">
            <Card className="border-vs-primary/20 bg-vs-primary/5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><Badge status={hasFindings ? "primary" : "neutral"}>{hasFindings ? "LATEST MISSION" : "MISSION COMPLETE · NO LIVE FINDINGS"}</Badge><p className="mt-2 text-sm text-vs-fg">For <strong>{missionTargetUser}</strong> in <strong>{missionGeography}</strong></p><p className="mt-1 text-xs text-vs-fg-muted">Completed {new Date(mission!.created_at).toLocaleDateString()}</p></div><Link href={`/venture/${venture.id}/research?again=1`} className="text-sm font-medium text-vs-primary">Run research again →</Link></div></Card>
