  ...
  return (
    <main className="mx-auto max-w-5xl p-6">
      ...
      <VentureModeSection mode="simple" className="mt-4">
        ...
        <h1 className="mt-2 text-3xl font-semibold text-vs-fg">What actually happened?</h1>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-vs-fg-muted">After you test or launch, come back and add real numbers. This is where the app learns the difference between what research and simulation suggested and what happened in real life.</p>
      </VentureModeSection>
      <VentureModeSection mode="pro" className="mt-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.2em] text-vs-primary">Learn</p><h1 className="mt-1 text-3xl font-semibold text-vs-fg">Reality vs the model</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-vs-fg-muted">Log what actually happens after launch. Real observations remain separate from simulated expectations, so variance can inform a future recalibration without rewriting history.</p></div><Badge status="success">REAL OBSERVATIONS ONLY</Badge></div></VentureModeSection>

      <VentureModeSection mode="simple" className="mt-6 space-y-4">
        <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          <Card><p className="text-xs font-semibold uppercase tracking-wide text-vs-fg-muted">Add something real</p><p className="mt-2 text-sm leading-6 text-vs-fg-muted">Examples: how many people tried it, monthly revenue, monthly cost, retention, or an important milestone.</p><div className="mt-4"><AddOutcomeForm ventureId={venture.id} /></div></Card>
          <Card><p className="text-xs font-semibold uppercase tracking-wide text-vs-fg-muted">Simulation vs real life</p>{!simulation ? <p className="mt-3 text-sm text-vs-fg-muted">You have not run a simulation yet, so there is nothing to compare.</p> : <div className="mt-4 space-y-3">{compareRows.map(([label, simulated, real]) => <SimpleCompare key={label} label={label} simulated={simulated} real={real} />)}</div>}<p className="mt-3 text-xs leading-5 text-vs-fg-muted">A difference does not automatically mean the simulation was “wrong.” It tells you which assumptions deserve another look.</p></Card>
        </section>
        {trackedMetrics.length > 0 && <Card><p className="mb-3 text-xs font-semibold uppercase tracking-wide text-vs-fg-muted">Your real progress</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2">{trackedMetrics.map((metric) => { const points = (byMetric.get(metric) ?? []).filter((o) => o.metric_value !== null).map((o) => ({ reportedAt: o.reported_at, value: o.metric_value as number })); return <OutcomeChart key={metric} title={METRIC_LABEL[metric] ?? metric} format={CURRENCY_METRICS.has(metric) ? "currency" : "number"} points={points} />; })}</div></Card>}
        {outcomes.length > 0 ? <Card><p className="text-xs font-semibold uppercase tracking-wide text-vs-fg-muted">What you have logged</p><ul className="mt-3 space-y-2">{[...outcomes].reverse().slice(0, 8).map((o) => <li key={o.id} className="rounded-vs-sm bg-vs-bg-subtle p-3 text-sm text-vs-fg-muted"><span className="font-medium text-vs-fg">{new Date(o.reported_at).toLocaleDateString()}</span> — {METRIC_LABEL[o.metric_type] ?? o.metric_type}: {o.metric_value ?? "—"}{o.note ? ` · ${o.note}` : ""}</li>)}</ul></Card> : <Card><p className="text-sm text-vs-fg-muted">Nothing real has been logged yet. That is normal if you have not launched or tested with people.</p></Card>}
        <Card className="border-vs-primary/30"><p className="text-lg font-semibold text-vs-fg">This closes the first learning loop.</p><p className="mt-2 text-sm leading-6 text-vs-fg-muted">Idea → research → shape → monetization → simulation → build → real results → better decisions. Earlier assumptions stay visible instead of being silently rewritten.</p><div className="mt-4 flex flex-wrap gap-3"><Link href={`/venture/${venture.id}/research`} className="rounded-vs-sm border border-vs-border px-4 py-2 text-sm font-semibold text-vs-fg">Revisit research</Link><Link href={`/venture/${venture.id}`} className="rounded-vs-sm bg-vs-primary px-4 py-2 text-sm font-semibold text-vs-primary-fg">Back to venture home</Link></div></Card>
      </VentureModeSection>

      <VentureModeSection mode="pro" className="mt-6 space-y-4">
        <section className="grid gap-4 lg:grid-cols-[1.2fr_1fr]"><Card><p className="text-xs font-semibold uppercase tracking-wide text-vs-fg-muted">Expectation vs observation</p>{!simulation ? <p className="mt-3 text-sm text-vs-fg-muted">No simulation exists yet, so there is no modeled expectation to compare.</p> : <div className="mt-4 space-y-3">{compareRows.map(([label, simulated, real]) => { const variance = real === null ? null : real - simulated; return <div key={label} className="grid grid-cols-3 gap-3 rounded-vs-md border border-vs-border p-3 text-sm"><div><p className="text-[10px] uppercase text-vs-fg-muted">Metric</p><p className="mt-1 font-medium text-vs-fg">{label}</p></div><div><p className="text-[10px] uppercase text-vs-fg-muted">Simulated</p><p className="mt-1 font-medium text-vs-fg">{simulated.toLocaleString()}</p></div><div><p className="text-[10px] uppercase text-vs-fg-muted">Real / variance</p><p className="mt-1 font-medium text-vs-fg">{real === null ? "Not logged" : `${real.toLocaleString()} (${variance! >= 0 ? "+" : ""}${variance!.toLocaleString()})`}</p></div></div>; })}</div>}<p className="mt-3 text-xs text-vs-fg-muted">Variance is descriptive only. It does not automatically change the simulator or claim causality.</p></Card><Card><p className="text-xs font-semibold uppercase tracking-wide text-vs-fg-muted">Add a real observation</p><div className="mt-3"><AddOutcomeForm ventureId={venture.id} /></div></Card></section>
        ...
        {outcomes.length > 0 ? <Card><p className="mb-2 text-sm font-semibold uppercase tracking-wide text-vs-fg-muted">Observation log</p><ul className="space-y-2">{[...outcomes].reverse().map((o) => <li key={o.id} className="rounded-vs-sm bg-vs-bg-subtle p-3 text-sm text-vs-fg-muted"><span className="font-medium text-vs-fg">{new Date(o.reported_at).toLocaleDateString()}</span> — {METRIC_LABEL[o.metric_type] ?? o.metric_type}: {o.metric_value ?? "—"}{o.note ? ` · ${o.note}` : ""}</li>)}</ul></Card> : <Card><p className="text-sm text-vs-fg-muted">No real-world observations logged yet.</p></Card>}
      </VentureModeSection>
    </main>
  );
}

function SimpleCompare({ label, simulated, real }: { label: string; simulated: number; real: number | null }) { const difference = real === null ? null : real - simulated; return <div className="rounded-vs-md border border-vs-border p-3"><div className="flex items-center justify-between gap-3"><p className="font-medium text-vs-fg">{label}</p><p className="text-xs text-vs-fg-muted">Simulation: {simulated.toLocaleString()}</p></div><p className="mt-2 text-sm text-vs-fg-muted">{real === null ? "Real result: not added yet" : `Real result: ${real.toLocaleString()} · ${difference! >= 0 ? "+" : ""}${difference!.toLocaleString()} from the simulation`}</p></div>; }
